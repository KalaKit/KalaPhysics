...
case "$1" in
    --linux)
        OUT_NAME=KalaPhysics-Linux

        LIB_NAME=libkalaphysics
        LIB_EXT=a
        LIB_ORIGIN=build/release-linux
        ;;
    --win-on-linux)
        OUT_NAME=KalaPhysics-Win-on-Linux

        LIB_NAME=kalaphysics-gnu
        LIB_EXT=lib
        LIB_ORIGIN=build/release-win-on-linux
        ;;
    --windows)
        OUT_NAME=KalaPhysics-Windows

        LIB_NAME=kalaphysics
        LIB_EXT=
